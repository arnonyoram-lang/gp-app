...
async function loadWa(){
  ...
  if(!hasBackend()){WA_PENDING=DEMO_WA.slice();renderWa();if(sum)_busyNote('וואטסאפ: דמו (אין גשר)');return;}
  try{const a=await gw({action:'waPending'});if(a&&a.ok){WA_PENDING=a.rows||[];renderWa();return;}WA_PENDING=[];renderWa();if(sum)sum.innerHTML='<div>⚠ '+esc((a&&a.error)||'שגיאה')+'</div>';}
  catch(e){WA_PENDING=[];renderWa();if(sum)sum.innerHTML='<div>⚠ שגיאת חיבור</div>';}
}
function renderWa(){
  const rows=WA_PENDING;
...
async function waApproveUI(id){
  ...
  try{const a=await gw({action:'waApprove',id:id});toast(a&&a.ok?('אושר ועבר ל'+(a.category=='עובדים זרים'?'לידים':'מועמדים')+' ✓'):'⚠ '+((a&&a.error)||''));loadWa();}catch(e){toast('שגיאת חיבור')}
}
async function waRejectUI(id){
  if(!hasBackend()){WA_PENDING=WA_PENDING.filter(r=>String(r['idMessage'])!=id);renderWa();toast('דמו: נדחה');return;}
