...
const SECTIONS=['home','intake','summary','followup','agreements','corp','active','commiss','manager','marketing','quality','knowledge','features'];
...
function computeTasks(){
  ...
  LEADS.forEach(r=>{try{
    ...
    const p=(txt,reason,lvl)=>tasks.push({c:String(r['חברה']||r['שם הלקוח']||'—'),txt,reason,lvl,owner:o,phone,score});
    if(dd!==null&&dd>0)p('פולואפ — תאריך עבר','מתי לפנות שוב: '+due,'hot');else if(dd===0)p('פולואפ להיום',' ','warn');
    if(r['דרישה חדשה']=='כן'||r['דרישה חדשה']===true)p('להציע עוד עובדים','דרישה חדשה','ok');
    if(r['חתם']=='כן'&&!r['תחילת עבודה'])p('לוודא התחלה מול תאגיד','חתום בלי תחילת עבודה','warn');
    if(st=='מתחמם'&&!due)p('ליד מתחמם — לקדם','מתחמם בלי תאריך','warn');
  }catch(e){}});
...
function renderFollowup(){
  ...
  document.getElementById('fuList').innerHTML=tasks.map((t,i)=>{
    ...
    return '<div class="task"><div class="b b-'+t.lvl+'"></div><div style="flex:1"><b>'+esc(t.c)+'</b> — '+esc(t.txt)+'<br><span class="muted">'+esc(t.reason)+'</span><div class="row" style="margin-top:6px"><button class="ghost" onclick="snoozeFU('+i+',7)">טופל (שבוע)</button><button class="ghost" onclick="snoozeFU('+i+',1)">דחה למחר</button><button class="ghost" onclick="openTreatFU('+i+')">טיפול</button>'+callBtn(t.phone)+wa+'</div></div><div style="text-align:center"><span class="pill" title="ניקוד ליד">'+t.score+'</span><br><span class="pill">'+esc(t.owner)+'</span></div></div>';
