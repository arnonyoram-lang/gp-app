...
async function agentRejectUI(row){if(!hasBackend())return;try{await gw({action:'agentReject',row:row});toast('נדחתה');_agentRefresh();}catch(e){toast('שגיאת חיבור')}}
...
function renderGuideZ(){
  ...
  '<h3>🏠 כרטיסי הבית — מה, מאיפה, לאן</h3>'+
  '<div style="font-size:13px;line-height:1.7">'+
  '<p><b>ליד חדש</b> → מסך קליטה. מקליד/מדביק טקסט חופשי (או קול) על פנייה. <b>"נתח"</b> = מחלץ שדות (שם, טלפון, כמות, מקצוע, לאום...) בעזרת AI/היוריסטיקה. עורך ידנית. <b>"שלח לאפשיט"</b> = כותב <b>ליד חדש לגיליון הלידים</b> (upsert — לא יוצר כפילות אם הטלפון קיים). מאיפה: מה שהקלדת. לאן: גיליון הלידים ← AppSheet.</p>'+
  '<p><b>סכם שיחה</b> → מזין טלפון + סיכום. <b>"נתח" → "עדכן"</b> = מוסיף להערות של הליד הקיים ומעדכן סטטוס. לא דורס הערות קיימות. לאן: שדה "הערות" של הליד בגיליון.</p>'+
  '<p><b>פולואפים להיום</b> → הרשימה החשובה. מאיפה: לידים שה"מתי לפנות שוב" שלהם הגיע/עבר, לפי ניקוד דחיפות. כפתורים לכל ליד: <b>חייג</b> (פותח חייגן), <b>וואטסאפ</b> (פותח שיחה ב-wa.me), <b>טופל (שבוע)</b> = דוחה את הליד לשבוע (כותב "מתי לפנות שוב"=+7), <b>סיים</b> = סוגר את המשימה (מנקה את הטריגר — הליד נשאר בגיליון, פשוט לא מציק), <b>טיפול</b> = פותח חלון: סוג בעיה + חומרה + מה נעשה → נשמר בהערות + קובע בדיקה חוזרת. לאן: הכל נכתב לשורת הליד בגיליון.</p>'+
  '<p><b>הסכמים תקועים</b> → לידים שקיבלו הסכם ולא חתמו/תקועים. כפתורי פעולה מעדכנים סטטוס בגיליון.</p>'+
  '<p><b>ממתינים לתאגיד</b> → חתמו אך עדיין לא הועברו לתאגיד/לא התחילו. פעולה = עדכון סטטוס בגיליון.</p>'+
  '<p><b>לקוחות פעילים</b> → עובדים כבר בשטח. תזכורות שביעות רצון. פעולה = עדכון בגיליון.</p>'+
  '<p><b>עמלות וגבייה</b> → מגיליון ה"שעות". מזהה פערים בין הערכת תשלום למה שהתקבל ("כסף על הרצפה"). <b>ייצוא Excel</b> = מוריד CSV. הערה: אם אין לך גישה לגיליון השעות — המסך יראה ריק/שגיאה.</p>'+
  '<p><b>דוח מנהל</b> → סיכום בוקר/סוף-יום (חמים, פולואפים, תקועים, פעילים). מחושב מהלידים בגיליון.</p>'+
  '<p><b>שיווק והודעות</b> → בוחר קהל → מקבל נוסחים מוכנים לכל ערוץ.</p>'+
  '<p><b>בקרת איכות</b> → סורק את הלידים ומראה בעיות (בלי טלפון, בלי סטטוס, כפילות, חתום בלי תאגיד...). זו בקרה — לא מתקן לבד.</p>'+
  '<p><b>מרכז ידע</b> → שאלות ותשובות/תסריטים מוכנים לשיחות. <b>פיצ\'רים</b> → מדריך קצר. <b>וואטסאפ נכנס</b> → ראה למטה.</p>'+
  '</div>'+

  '<h3>📩 וואטסאפ נכנס — כפתור-כפתור</h3>'+
  '<div style="font-size:13px;line-height:1.7">'+
  '<p><b>משוך עכשiו</b> = מושך הודעות חדשות מגרין (24 שעות אחרונות), מסווג (זרים/מנהלים/אחר), ומכניס ל"ממתין". <b>משיכה בלבד — לא שולח כלום.</b></p>'+
  '<p><b>רענן ממתינים (מקור+שם)</b> = מנקה את הממתינים ומושך מחדש את הכל מהחלון (כדי להביא מקור/שם ולהסיר מושתקות). <b>⏱️ משיכה אוטומטית</b> = מפעיל/מכבה משיכה שעתית אוטומטית. <b>בדיקת חיבור</b> = בודק שהחיבור לגרין תקין.</p>'+
  '<p><b>מתג "ממתין / מאושר":</b> "ממתין" = חדשות שטרם החלטת עליהן. "מאושר" = שאישרת אך עדיין לא העברת לגיליון.</p>'+
  '<p><b>על כל הודעה:</b> <b>אשר</b> = שומר אצלנו ברשימת "מאושר" (לא נכתב לשום מקום עדיין). <b>דחה</b> = מסיר. <b>🔇 השתק קבוצה / החרג</b> = מוסיף את הקבוצה/איש-הקשר לרשימת החרגה + מוחק את ההודעות הקיימות ממנו. הודעה "אחר" מקבלת בחירה: <b>אשר→ליד</b> או <b>אשר→מועמד</b>.</p>'+
  '<p><b>בלשונית "מאושר":</b> <b>העבר לגוגל שיט</b> = כאן זה באמת נכתב — עובדים זרים → <b>גיליון הלידים</b>, מנהלי עבודה → <b>גיליון המועמדים</b>. רק הצעד הזה כותב לאמת.</p>'+
  '<p><b>📝 בקשות והערות לקלוד</b> = תיבה לצבור בקשות/שמות קבוצות. נשמר בגיליון, ואני קורא כשתגיד "תטפל בבקשות".</p>'+
  '</div>'+

  '<h3>🔄 הזרימה המלאה (מקצה לקצה)</h3>'+
  '<div style="font-size:13px;line-height:1.7">'+
  '<p>1. <b>פנייה נכנסת</b> (טלפון/וואטסאפ) → 2. <b>וואטסאפ נכנס</b> מושך אוטומטית ומסווג → 3. אתה <b>מאשר</b> → 4. <b>מעביר לגוגל שיט</b> → נוצר <b>ליד בגיליון</b> → 5. הליד מופיע ב<b>פולואפים</b> → 6. אתה מתקשר/שולח הסכם (חייג/וואטסאפ), מעדכן סטטוס (טופל/סיים/טיפול) → 7. חתם → <b>ממתינים לתאגיד</b> → 8. הועבר לתאגיד → 9. <b>לקוחות פעילים</b> + מעקב שעות ב<b>עמלות וגבייה</b>. כל שלב נכתב לגיליון ← AppSheet.</p>'+
  '</div>'+
...
function renderGuideM(){
  ...
  '<h3>🏠 הבית של מנהלי עבודה — מה, מאיפה, לאן</h3>'+
  '<div style="font-size:13px;line-height:1.7">'+
  '<p><b>פולואפ פתוחים</b> → מאחד את מי שצריך טיפול: <b>מועמדים בלי ראיון</b> + <b>קבלנים פתוחים</b>. כל שורה: פעולה/הערה · חייג · וואטסאפ. לאן: נכתב לשורת המועמד/קבלן בגיליון.</p>'+
...
function renderFeatures(){
  const A=[
    ...
    ['🔇 השתקת קבוצות ואנשים','כפתור על כל הודעה — משתיק קבוצה/איש קשר מיד ומוחק את מה שכבר נכנס.','רק מה שרלוונטי מגיע אליך.'],
