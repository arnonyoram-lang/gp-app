...
const SECTIONS=['home','intake','summary','followup','agreements','corp','active','commiss','manager','marketing','quality','knowledge','features','mgr','wa','guideZ','guideM','clients','more','control'];
...
function renderControl(){
  loadAll(async function(){
    ...
    let waN=0;if(hasBackend()){try{const a=await gw({action:'waPending'});if(a&&a.ok)waN=(a.rows||[]).length;}catch(e){}}
    const sum=document.getElementById('controlSummary');if(sum)sum.innerHTML='<div><b>'+tasks.length+'</b> פולואפים</div><div>🔴 '+tasks.filter(t=>t.lvl=='hot').length+' דחופים</div>'+(waN?'<div>📩 '+waN+' וואטסאפ</div>':'')+(stuck.length?'<div>⚠ '+stuck.length+' תקועים</div>':'');
    let ban='';
