...
const SECTIONS=['home','intake','summary','followup','agreements','corp','active','commiss','manager','marketing','quality','knowledge','features','mgr','wa','guideZ','guideM','clients','more','control','intakeHub','insights'];
...
function renderControl(){
  loadAll(async function(){
    ...
    el.innerHTML=ban+tasks.map((t,i)=>{
      const wa=t.phone?'<button class="ok" onclick="waSend(\''+String(t.phone).replace(/\D/g,'')+'\')">וואטסאפ</button>':'';
      return '<div class="task"><div class="b b-'+t.lvl+'"></div><div style="flex:1"><b>'+esc(t.c)+'</b> — '+esc(t.txt)+'<br><span class="muted">'+esc(t.reason)+'</span><div class="row" style="margin-top:6px"><button class="ghost" onclick="ctlSnooze('+i+')">טופל (שבוע)</button><button class="ghost" onclick="ctlClose('+i+')">סיים</button><button class="ghost" onclick="ctlTreat('+i+')">טיפול</button><button class="ghost" onclick="openHistory(\''+esc(String(t.phone))+'\',\''+esc(t.c)+'\')">📜 היסטוריה</button>'+callBtn(t.phone)+wa+'</div></div><div style="text-align:center"><span class="pill">'+t.score+'</span></div></div>';
...
async function agentApproveUI(row){if(!hasBackend())return;try{const a=await gw({action:'agentApprove',row:row});toast(a&&a.ok?'הצעת הסוכן אושרה ✓':'⚠');renderControl();}catch(e){toast('שגיאת חיבור')}}
async function agentRejectUI(row){if(!hasBackend())return;try{await gw({action:'agentReject',row:row});toast('נדחתה');renderControl();}catch(e){toast('שגיאת חיבור')}}
