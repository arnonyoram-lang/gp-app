...
const SECTIONS=['home','intake','commiss','manager','marketing','quality','knowledge','mgr','wa','guideZ','guideM','clients','more','control','intakeHub','insights'];
...
function show(id,_back){
  ...
  _updBackBtn();
  const map={commiss:renderCommiss,manager:renderManager,quality:renderQuality};
...
function openTreat(phone,name){
  ...
  bg.innerHTML='<div class="modal"><b>טיפול / סכם שיחה: '+esc(name||'')+'</b>'+
    ...
    '<label>מתי לחזור ללקוח (ימים)</label><input id="trDays" type="number" value="3">'+
    '<div class="row"><button class="ok" onclick="saveTreat(\''+String(phone).replace(/\D/g,'')+'\')">שמור</button><button class="ghost" onclick="this.closest(\'.modal-bg\').remove()">בטל</button></div>'+
    '<p class="muted">נשמר אצל הלקוח (סטטוס + הערות + מתי לפנות שוב) — ומסתנכרן ל-AppSheet.</p></div>';
  document.body.appendChild(bg);
}
async function saveTreat(phone){
  if(_saving)return;
  const status=val('trStatus'),type=val('trType'),text=document.getElementById('trText').value.trim(),days=parseInt(document.getElementById('trDays').value)||3;
  if(!text&&!status){toast('כתוב מה נעשה או עדכן סטטוס');return}
  ...
  else toast('דמו: היה נשמר אצל הלקוח');
  logAct(phone,'ליד','טיפול/שיחה',line);
  _saving=false;
...
function renderGuideZ(){
  ...
  '<p><b>🎯 מוקד היום</b> → מסך העבודה הראשי: <b>לעבוד לפי דחיפות</b>. מציג את כל מי שצריך טיפול עכשיו — פולואפים מדורגים + באנר וואטסאפ ממתין + תקועים + 🤖 תיבת הסוכן + באנר <b>💬 "ענו לך"</b> (מי שהשיב בוואטסאפ קופץ לראש), וגם <b>"✅ טיפלת היום ב-X"</b>. כפתורים לכל ליד: <b>טופל (שבוע)</b> = דוחה שבוע (הכרטיס יורד מיד, הסנכרון ברקע), <b>סיים</b> = סוגר את המשימה (הליד נשאר בגיליון), <b>טיפול</b> = תיעוד שיחה: סטטוס+מה נעשה+מתי לחזור, <b>→ כרטיס מלא</b>, <b>📜 היסטוריה</b>, <b>חייג/וואטסאפ</b>.</p>'+
  '<p><b>👥 לקוחות</b> → <b>לחפש ולנהל</b>: כל לקוח (גם סגור), חיפוש שם/חברה/טלפון, מיון א-ב או מעודכנים, 📜 היסטוריה מלאה + טיפול.</p>'+
  '<p><b>📥 קליטה</b> → <b>ליד חדש</b> (הדבק טקסט → "נתח" → "שלח לאפשיט"; לא יוצר כפילות על טלפון קיים ולא דורס הערות/סטטוס), ו<b>וואטסאפ נכנס</b> (ראה למטה). <b>לתעד שיחה עם לקוח קיים</b> = פותחים אותו ולוחצים <b>"טיפול"</b> (מעדכן סטטוס + מצרף הערה + קובע מתי לחזור, עם 🎤 הקראה).</p>'+
  '<p><b>📊 תובנות</b> → <b>דוח מנהל</b> (KPI + פייפליין + 📈 תוצאות: טופל היום/השבוע, זמן תגובה, 🎯 משפך מול יעד 6, לידים בלי מענה 24ש\'+), <b>עמלות וגבייה</b> (מגיליון "שעות"; אם לא זמין — באנר ⛔ ומספרים דמו), <b>בקרת איכות</b>.</p>'+
  '<p><b>⋯ עוד</b> → שיווק, מרכז ידע, מדריך, ו<b>👤 מי משתמש עכשיו</b> — בוחרים יורם/עוזר/אביבית כדי שכל פעולה תירשם על שם מי שעשה אותה.</p>'+
  '</div>'+
  ...
  '<p><b>⏱️ משיכה אוטומטית</b> = משיכה שעתית. הכפתור מוריק כשפעיל, ו<b>מאדים ⚠</b> אם המשיכה נכשלת (גם מייל התראה נשלח אליך). <b>🔇 מושתקים</b> = רשימת הקבוצות/אנשים שהשתקת + כפתור ביטול השתקה. תחת <b>⚙ כלים</b>: "בדיקת חיבור" ו"רענן ממתינים" (שימוש נדיר).</p>'+
  ...
  '<div style="font-size:13px;line-height:1.7">'+
  '<p>1. <b>פנייה נכנסת</b> (טלפון/וואטסאפ) → 2. <b>וואטסאפ נכנס</b> מושך אוטומטית ומסווג (והסוכן מציע סיווג ל"אחר") → 3. אתה <b>מאשר</b> → 4. <b>מעביר לגוגל שיט</b> → נוצר <b>ליד בגיליון</b> (עם מזהה ייחודי) → 5. הליד קופץ מיד ל<b>🎯 מוקד היום</b> → 6. אתה מתקשר/שולח הסכם ומעדכן בכל שיחה דרך <b>טיפול</b> (הסטטוס עולה: קר→מתחמם→בעבודה, ונקבע מתי לחזור) → 7. <b>חתם + תחילת עבודה</b> — מי שחתם בלי תחילת עבודה מופיע כ"תקוע" במוקד → 8. לקוח פעיל + מעקב שעות ב<b>עמלות</b>. כל שלב נכתב לגיליון ← AppSheet, ונרשם ב<b>📜 היסטוריה</b>, ונספר ב<b>🎯 משפך</b> שבדוח מנהל.</p>'+
  '</div>'+
  ...
  '<p>• <b>סוכן שמנהל שיחה שלמה</b> — לא רק מנסח, אלא עוקב אחרי תשובת הלקוח וממשיך את השיחה עד לתיאום/סגירה (באישורך).</p>'+
  '<p>• <b>ניקוד "חום" חי</b> — לתעדף את מי שהכי קרוב לחתום, אוטומטית.</p>'+
  '</div>'+
  '<div class="row" style="margin-top:12px"><button class="ghost" onclick="show(\'home\')">← חזרה לבית</button></div>';
}
