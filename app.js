...
const ROLE_SECTIONS={'עוזר':['intake','summary','agreements','corp','active','knowledge'],'אביבית':['marketing','commiss','quality','manager','knowledge']};
...
// בורר תפקיד: קובע 'אחראי' לכל פעולה + יומן, ומסנן מסכים לפי ROLE_SECTIONS
function setRole(name){const c=cfg();c.owner=name;c.role=name;try{localStorage.setItem(CFG_KEY,JSON.stringify(c))}catch(e){}applyRole();toast(name?('שלום '+name+' 👋 — הפעולות יירשמו על שמך'):'ללא סינון — כל המסכים מוצגים')}
...
const WRITE_ACTIONS=['append','update','upsert','appendNote','waApprove','waApproveMany','waPush','waPushMany','waReject','waRejectMany','waMoveMany','waResetPending','waPull','waNote','waNoteDone','waMute','waUnmute','waMove','waAutoOn','waAutoOff','logActivity','agentApprove','agentReject','agentDraft','agentClassify'];
...
const SECTIONS=['home','intake','summary','agreements','corp','active','commiss','manager','marketing','quality','knowledge','features','mgr','wa','guideZ','guideM','clients','more','control','intakeHub','insights'];
let _navHist=[]; // מחסנית ניווט — לכפתור "← חזור"
function _curSection(){return SECTIONS.find(s=>{const e=document.getElementById(s);return e&&!e.classList.contains('hide')})}
function goBack(){const prev=_navHist.pop();show(prev||'home',true);}
function _updBackBtn(){const b=document.getElementById('backBtn');if(b)b.style.display=_navHist.length?'':'none';}
function show(id,_back){
  if(!_back){const cur=_curSection();if(cur&&cur!==id){_navHist.push(cur);if(_navHist.length>50)_navHist.shift();}}
  SECTIONS.forEach(s=>document.getElementById(s).classList.add('hide'));document.getElementById(id).classList.remove('hide');
  _updBackBtn();
  const map={agreements:renderAgreements,corp:renderCorp,active:renderActive,commiss:renderCommiss,manager:renderManager,quality:renderQuality};
  if(map[id])loadAll(map[id]);
  if(id=='marketing')initMarketing();
  if(id=='knowledge')initKnowledge();
  if(id=='features')renderFeatures();
  if(id=='mgr')mgrReload();
  if(id=='wa'){loadWa();loadWaNotes();waAutoRefresh();}
  if(id=='control')renderControl();
  if(id=='clients')loadAll(renderClients);
  if(id=='guideZ')renderGuideZ();
  if(id=='guideM')renderGuideM();
  if(id=='home'){if(!_back)_navHist=[];_updBackBtn();setMode(appMode);if(appMode=='z')renderHome();}
...
function _isoLocal(d){return d.getFullYear()+'-'+('0'+(d.getMonth()+1)).slice(-2)+'-'+('0'+d.getDate()).slice(-2)}
function todayISO(){return _isoLocal(new Date())}
function plusDays(n){const d=new Date();d.setDate(d.getDate()+n);return _isoLocal(d)}
...
async function sendIntake(){if(_saving)return;const j=buildIntakeJson();if(!hasBackend()){alert('אין חיבור — השתמש ב"העתק JSON".');return}_saving=true;try{const res=await gw(j);if(res.ok){try{localStorage.removeItem('gp_draft')}catch(e){}}alert(res.ok?'נשמר ✓ ('+(res.mode||'')+')':'שגיאה: '+res.error)}catch(e){alert('שגיאת חיבור. נסה "העתק JSON". '+e.message)}finally{_saving=false}}
...
function buildSumJson(np){const set={};document.querySelectorAll('#sumResult [data-k]').forEach(el=>{if(el.dataset.k!='risk')set[el.dataset.k]=el.value});set['עדכון אחרון']=todayISO();const j={action:'update',table:cfg().t1||'',sheetId:leadsId(),key:leadKey(np),set};document.getElementById('sumJson').textContent=JSON.stringify(j,null,2);return j}
async function sendSummary(np){
  ...
  }catch(e){alert('שגיאת חיבור. '+e.message)}finally{_saving=false}
}
...
let _ctlDone=null; // כמה טופלו היום (מיומן התיעוד) — מוצג כמד התקדמות
...
// רענון המסך הפעיל אחרי פעולה (מוקד היום / לקוחות)
function refreshActive(){
  const c=document.getElementById('control');if(c&&!c.classList.contains('hide')){renderControl();return;}
  const cl=document.getElementById('clients');if(cl&&!cl.classList.contains('hide')){renderClients();return;}
}
...
// קפיצה מ"מוקד היום" לכרטיס המלא של הלקוח/רשומה במסך "לקוחות"/"מאגר"
function openClientCard(phone,name){const q=String(phone||name||'');const s=document.getElementById('clientSearch');if(s)s.value=q;show('clients');const s2=document.getElementById('clientSearch');if(s2)s2.value=q;renderClients();}
function openMgrCard(view,name){mgrView=view;const s=document.getElementById('mgrSearch');if(s)s.value=String(name||'');show('mgr');}
...
function renderHome(){loadAll(function(){
  ...
  el.innerHTML='<div class="card" style="border-color:var(--accent)"><b>מה עכשיו?</b>'+
    ...
    '<div class="row"><button class="primary" onclick="show(\'control\')">פתח מוקד היום ('+tasks.length+')</button></div></div>';
})}
...
async function loadWa(){
  ...
  if(!hasBackend()){WA_ROWS=((waView=='appr')?[]:DEMO_WA.slice()).filter(waModeMatch);renderWa();if(sum)_busyNote('וואטסאפ: דמו (אין גשר)');return;}
  const action=(waView=='appr')?'waApproved':'waPending';
  try{const a=await gw({action:action});if(a&&a.ok){WA_ROWS=(a.rows||[]).filter(waModeMatch);renderWa();return;}WA_ROWS=[];renderWa();if(sum)sum.innerHTML='<div>⚠ '+esc((a&&a.error)||'שגיאה')+'</div>';}
  catch(e){WA_ROWS=[];renderWa();if(sum)sum.innerHTML='<div>⚠ שגיאת חיבור</div>';}
}
// הפרדה לפי מצב: מנהלים רואים "מנהלי עבודה"; זרים רואים "עובדים זרים" + "אחר"
function waMode(){return (typeof appMode!=='undefined'&&appMode=='m')?'m':'z';}
function waModeMatch(r){const c=String(r['קטגוריה']||'');return waMode()=='m'?(c=='מנהלי עבודה'):(c=='עובדים זרים'||c=='אחר');}
...
function renderWaPending(){
  ...
  document.getElementById('waList').innerHTML=_waGroup(rows).map(g=>{
    ...
    const grpName=src.replace('קבוצה: ','').trim();
    const person=String(g.name||'').trim()||String(g.phone||'').replace(/\D/g,''); // שם השולח, ואם אין — הטלפון שלו
    const personLbl=String(g.name||'').trim()||String(g.phone||'');
...
let _waActing=false;
async function waMuteUI(name,kind,label){
  if(!hasBackend()){toast('דמו');return;}if(!name)return;
  const disp=label||name;
  const msg=kind=='person'?('להחריג את "'+disp+'"? הודעות מהאדם הזה לא ייכנסו — גם מתוך קבוצות.'):('להשתיק את "'+disp+'"? הודעות מהקבוצה לא ייכנסו יותר.');
  if(!confirm(msg+' (אפשר לבטל דרך 🔇 מושתקים)'))return;
  try{const a=await gw({action:'waMute',name:name});toast(a&&a.ok?('"'+disp+'" '+(kind=='person'?'הוחרג':'הושתק')+' ✓'):'⚠ '+((a&&a.error)||''));loadWa();}catch(e){toast('שגיאת חיבור')}
}
...
function renderClients(){
  ...
  const nameOf=r=>String(r['חברה']||r['שם הלקוח']||'');
  let rows=LEADS.filter(r=>r['חברה']||r['שם הלקוח']||r['טלפון']);
...
function renderGuideZ(){
  document.getElementById('guideZContent').innerHTML=
  '<h2 style="margin:0 0 4px">📖 מדריך מלא — עובדים זרים</h2>'+
  '<p class="muted" style="margin:0 0 12px">כל כפתור וכל זרימה: מאיפה מגיע כל דבר ולאן הוא הולך.</p>'+

  '<h3>🧭 העיקרון</h3>'+
  '<p><b>מקור האמת = AppSheet + Google Sheets</b> (גיליון "מתעניינים בעובדים זרים" + גיליון "שעות"). האפליקציה הזו היא <b>שכבת פעולה</b>: קוראת מהגיליון וכותבת אליו בחזרה. כל שינוי כאן נשמר בגיליון → ומשם מסתנכרן ל-AppSheet. אנחנו לא מאגר שני — רק שכבה חכמה מעל.</p>'+
  '<p><b>מתג עליון (בבית):</b> "עובדים זרים" ⇄ "מנהלי עבודה" — מחליף את כל התצוגה בין שני העסקים.</p>'+

  '<h3>🏠 הבית — 4 מוקדים + עוד</h3>'+
  '<div style="font-size:13px;line-height:1.7">'+
  '<p><b>🎯 מוקד היום</b> → מסך העבודה הראשי: <b>לעבוד לפי דחיפות</b>. מציג את כל מי שצריך טיפול עכשיו — פולואפים מדורגים + באנר וואטסאפ ממתין + תקועים + 🤖 תיבת הסוכן, וגם <b>"✅ טיפלת היום ב-X"</b>. כפתורים לכל ליד: <b>טופל (שבוע)</b> = דוחה שבוע (הכרטיס יורד מיד, הסנכרון ברקע), <b>סיים</b> = סוגר את המשימה (הליד נשאר בגיליון), <b>טיפול</b> = בעיה+חומרה+מה נעשה → להערות, <b>→ כרטיס מלא</b> = קופץ ללקוח ב"לקוחות", <b>📜 היסטוריה</b>, <b>חייג/וואטסאפ</b>. לאן: הכל לשורת הליד בגיליון.</p>'+
  '<p><b>👥 לקוחות</b> → <b>לחפש ולנהל</b>: כל לקוח (גם סגור), חיפוש שם/חברה/טלפון, מיון א-ב או מעודכנים, 📜 היסטוריה מלאה + טיפול.</p>'+
  '<p><b>📥 קליטה</b> → <b>ליד חדש</b> (הדבק טקסט → "נתח" → "שלח לאפשיט"; לא יוצר כפילות על טלפון קיים), <b>סכם שיחה</b> (טלפון+סיכום → "עדכן" מצרף להערות בלי לדרוס), <b>וואטסאפ נכנס</b> (ראה למטה).</p>'+
  '<p><b>📊 תובנות</b> → <b>דוח מנהל</b> (KPI + פייפליין + 📈 תוצאות: מה טופל היום/השבוע, זמן תגובה, לידים בלי מענה 24ש\'+), <b>עמלות וגבייה</b> (מגיליון "שעות"; אם הגיליון לא זמין — באנר ⛔ והמספרים דמו), <b>בקרת איכות</b>.</p>'+
  '<p><b>⋯ עוד</b> → שיווק, מרכז ידע, מדריך, פיצ\'רים, מסכי "לפי שלב" (הסכמים/תאגיד/פעילים), ו<b>👤 מי משתמש עכשיו</b> — בוחרים יורם/עוזר/אביבית כדי שכל פעולה תירשם על שם מי שעשה אותה.</p>'+
  '</div>'+

  '<h3>📩 וואטסאפ נכנס — כפתור-כפתור</h3>'+
  '<div style="font-size:13px;line-height:1.7">'+
  '<p><b>משוך עכשיו</b> = מושך הודעות חדשות מגרין (24 שעות אחרונות), מסווג (זרים/מנהלים/אחר), ומכניס ל"ממתין". <b>משיכה בלבד — לא שולח כלום.</b> כל תחום רואה רק את שלו: בזרים — זרים+אחר; במנהלים — מנהלים. <b>↪ העבר</b> = מעביר הודעה לתחום השני.</p>'+
  '<p><b>⏱️ משיכה אוטומטית</b> = משיכה שעתית. הכפתור מוריק כשפעיל, ו<b>מאדים ⚠</b> אם המשיכה נכשלת (גם מייל התראה נשלח אליך). <b>🔇 מושתקים</b> = רשימת הקבוצות/אנשים שהשתקת + כפתור ביטול השתקה. <b>בדיקת חיבור</b> = בודק את גרין. <b>רענן ממתינים</b> = מושך הכל מחדש.</p>'+
  '<p><b>על כל שולח (הודעות מקובצות):</b> <b>אשר</b> = לרשימת "מאושר" (עדיין לא נכתב לגיליון). <b>דחה</b> = מסיר (עם שאלת אישור). <b>🔇 השתק/החרג</b> = חוסם את הקבוצה/האיש (עם אישור, ניתן לביטול). הודעת "אחר": <b>אשר→ליד</b> או <b>אשר→מועמד</b>. כל פעולה על קבוצה = בבת אחת.</p>'+
  '<p><b>בלשונית "מאושר":</b> <b>העבר לגוגל שיט</b> = כאן זה באמת נכתב — זרים → <b>גיליון הלידים</b> (הליד קופץ מיד למוקד היום), מנהלים → <b>גיליון המועמדים</b>. רק הצעד הזה כותב לאמת.</p>'+
  '<p><b>🤖 הסוכן המסווג:</b> אחרי כל משיכה, הודעות "אחר" נשלחות ל-AI. כשהוא בטוח (80%+) הוא <b>מציע</b> סיווג בתיבת הסוכן במוקד היום — <b>אישור שלך מבצע</b> את הסיווג (רק על רשימת ההמתנה, לא על הגיליונות). דחייה = כלום לא קורה.</p>'+
  '<p><b>📝 בקשות והערות לקלוד</b> = תיבה לצבור בקשות. נשמר בגיליון, ואני קורא כשתגיד "תטפל בבקשות".</p>'+
  '</div>'+

  '<h3>🔄 הזרימה המלאה (מקצה לקצה)</h3>'+
  '<div style="font-size:13px;line-height:1.7">'+
  '<p>1. <b>פנייה נכנסת</b> (טלפון/וואטסאפ) → 2. <b>וואטסאפ נכנס</b> מושך אוטומטית ומסווג (והסוכן מציע סיווג ל"אחר") → 3. אתה <b>מאשר</b> → 4. <b>מעביר לגוגל שיט</b> → נוצר <b>ליד בגיליון</b> → 5. הליד קופץ מיד ל<b>🎯 מוקד היום</b> → 6. אתה מתקשר/שולח הסכם, מעדכן (טופל/סיים/טיפול) → 7. חתם → <b>ממתינים לתאגיד</b> → 8. הועבר לתאגיד → 9. <b>לקוחות פעילים</b> + מעקב שעות ב<b>עמלות</b>. כל שלב נכתב לגיליון ← AppSheet, ונרשם ב<b>📜 היסטוריה</b>.</p>'+
  '</div>'+

  '<h3>💡 הרעיונות שלי לשיפור (זרים)</h3>'+
  '<div style="font-size:13px;line-height:1.8">'+
  '<p>• <b>ניקוד ליד חכם יותר</b> — לתעדף פולואפים לפי סיכוי סגירה (מקצוע מבוקש + מהירות תגובה + היסטוריה), לא רק תאריך.</p>'+
  '<p>• <b>תבניות תשובה מהירות בוואטסאפ</b> — כפתור ששולח (באישורך) נוסח מוכן ללקוח ("קיבלתי, נחזור אליך היום") — כשנפתח שליחה.</p>'+
  '<p>• <b>התראת "כסף על הרצפה"</b> — כשפער גבייה עובר סכום, התראה יזומה (מייל/וואטסאפ).</p>'+
  '<p>• <b>מזהה ליד אמיתי</b> — לעדכן לפי ID ולא לפי טלפון (טלפון יכול לחזור על עצמו).</p>'+
  '<p>• <b>סיכום קולי אוטומטי אחרי שיחה</b> — להקליט/להכתיב → סיכום מסודר להערות בלחיצה.</p>'+
  '<p>• <b>לוח מחוונים יומי</b> — כמה נסגרו, כמה בסיכון, יעד 6 לקוחות/חודש — במבט אחד.</p>'+
  '</div>'+
  '<div class="row" style="margin-top:12px"><button class="ghost" onclick="show(\'home\')">← חזרה לבית</button></div>';
}
function renderGuideM(){
  document.getElementById('guideMContent').innerHTML=
  '<h2 style="margin:0 0 4px">📖 מדריך שימוש — מנהלי עבודה (השמה)</h2>'+
  '<p class="muted" style="margin:0 0 12px">כל כפתור וכל זרימה: מאיפה ולאן — עד רמת הראיון.</p>'+

  '<h3>🧭 העיקרון</h3>'+
  '<p><b>מקור האמת = גיליון "מנהלי עבודה להשמה" (בבעלותך) + אפליקציית הראיונות + AppSheet.</b> האפליקציה הזו היא <b>שכבת פעולה בלבד</b> — היא <b>לא</b> מנהלת ראיונות ולא עושה התאמות (זה באפליקציית הראיונות). היא דוחפת פתוחים, קולטת מוואטסאפ, ומעדכנת חזרה לגיליון/ראיונות.</p>'+
  '<p><b>שוק דו-צדדי:</b> <b>מזמין/מעסיק (הקבלן שמשלם)</b> ↔ <b>מועמד (מנהל העבודה)</b>. המבנה: <b>קבלן → פרויקט → השמה</b> (ההשמה מחברת פרויקט למועמד ונושאת את ה<b>עמלה</b>).</p>'+

  '<h3>🏠 הבית של מנהלי עבודה — 4 מוקדים</h3>'+
  '<div style="font-size:13px;line-height:1.7">'+
...
function renderFeatures(){
  const A=[
    ...
    ['🎯 מוקד היום','כל מי שצריך טיפול עכשיו: פולואפים מדורגים + וואטסאפ ממתין + תקועים + תיבת הסוכן. "טופל" מוריד את הכרטיס מיד.','מסך עבודה אחד — יודעים בדיוק על מי להתקשר, בלי המתנות.'],
    ['✅ מונה תוצאות יומי','"טיפלת היום ב-X" במוקד + 📈 תוצאות בדוח מנהל (טופלו השבוע, זמן תגובה, לידים בלי מענה 24ש\'+).','רואים תוצאות, לא רק רשימות.'],
    ['🤖 סוכן מסווג + תיבת אישור','AI מסווג הודעות "אחר" ומציע; אישור שלך מבצע (רק פעולות מהרשימה המותרת). הכל מתועד.','העבודה השחורה נעשית לבד — אתה רק מאשר.'],
    ...
    ['🔎 חיפוש לקוח + היסטוריה','חפש כל לקוח (שם/חברה/טלפון, גם אחרי "סיים") לפי א-ב או מעודכנים אחרונה ← 📜 היסטוריה = כל הפעולות לאורך זמן.','זיכרון ארגוני מלא — שום פנייה לא נעלמת.'],
    ['📜 תיעוד אוטומטי לאורך זמן','כל פעולה (טופל/סיים/טיפול/עדכון/פעולה על מועמד/קבלן) נרשמת ביומן עם תאריך ומי עשה.','רואים את כל הסיפור של כל לקוח — לא רק את הסטטוס.'],
    ['📩 וואטסאפ נכנס — חי','משיכה שעתית אוטומטית (או ידנית), סיווג זרים/מנהלים/אחר, קיבוץ לפי שולח, אישור←ליד/מועמד. משיכה בלבד — לא שולח כלום.','פניות נכנסות לבד, אתה רק מאשר.'],
    ['🔇 השתקת קבוצות ואנשים','כפתור על כל הודעה (עם אישור) + מסך "מושתקים" לביטול השתקה.','רק מה שרלוונטי מגיע אליך — והכל הפיך.'],
    ['⇄ שני עסקים במתג','מתג עליון עובדים זרים ⇄ מנהלי עבודה — לכל אחד בית ומסכים משלו (מועמדים/קבלנים/פרויקטים/השמות).','מרכז פעולה אחד לשני העסקים.'],
    ['📖 מדריך שימוש מלא','מדריך מפורט לכל תחום — כל כפתור וכל זרימה + רעיונות שיפור.','אתה והצוות יודעים בדיוק מה עושה כל דבר.'],
    ['📝 בקשות והערות לקלוד','תיבה לצבור בקשות/תיקונים ← "תטפל בבקשות" ואני קורא ומטפל בבת אחת.','לא שוכחים כלום.'],
    ['כלים נוחים','הקראה קולית, מצב כהה, מובייל, בורר "מי משתמש עכשיו" (עוד), שמירת טיוטה, גיבוי יומי אוטומטי.','עבודה מהירה גם מהשטח ומהנייד.']
  ];
  const F=[
    ['חילוץ AI מדויק','מפתח Anthropic ← חילוץ חכם יותר מטקסט חופשי, פחות עריכה ידנית.'],
    ['מזהה ייחודי לעדכונים','לעדכן לפי ID ולא לפי טלפון — למניעת עדכון שורה שגויה.'],
    ['הצעת מועמד לפרויקט (מנהלים)','מפרויקט פתוח ← מועמדים מתאימים לפי אזור/הסמכה ← דחיפה לראיונות.'],
    ['התראות יזומות','כשליד נהיה חם, הסכם נתקע, או עמלה לא חויבה/שולמה.'],
    ['חיבור גיליון השעות','כדי שמסך העמלות יציג נתונים אמיתיים (לשתף את "שעות" עם החשבון שלך).'],
    ['סטטוס וציון ראיון מוצג','למשוך מאפליקציית הראיונות ולהציג ליד המועמד.'],
    ['שליחת וואטסאפ מאושרת','תבניות תשובה מוכנות שנשלחות באישורך (כרגע כבוי בכוונה).']
  ];
