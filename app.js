...
async function gw(payload){
  ...
  try{
    ...
    const c=cfg();if(!c.url)throw new Error('no_gateway');
    const r=await fetch(c.url,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({token:c.token,...payload})});return await r.json();
  }finally{busy(false)}
}

const SECTIONS=['home','intake','summary','followup','agreements','corp','active','commiss','manager','marketing','quality','knowledge','features','mgr'];
...
function mgrReload(){
  [['mgrTabCand','cand'],['mgrTabCon','con'],['mgrTabProj','proj'],['mgrTabPlace','place']].forEach(function(x){const b=document.getElementById(x[0]);if(b)b.className=(mgrView==x[1]?'primary':'ghost')});
  if(mgrView=='con')loadCon();else if(mgrView=='proj')loadProj();else if(mgrView=='place')loadPlace();else loadMgr();
}
async function loadCon(){
  if(hasBackend()){try{const a=await gw({action:'get',sheetId:SHEET_MANAGERS,gid:CONTRACTORS_GID});if(a&&a.ok){CON=a.rows;renderMgr();return}else if(a&&a.error){CON=DEMO_CON;renderMgr();const bz=document.getElementById('busy');if(bz){bz.style.display='inline-block';bz.textContent='⚠ קבלנים: '+a.error}return}}catch(e){}}
  CON=DEMO_CON;renderMgr();
}
function renderMgr(){ if(mgrView=='con')renderContractors(); else if(mgrView=='proj')renderProjects(); else if(mgrView=='place')renderPlacements(); else renderCandidates(); }
function renderContractors(){
  const q=((document.getElementById('mgrSearch')||{}).value||'').trim();
  let rows=CON.filter(r=>r['שם חברה']||r['איש קשר']);
...
let PROJ=[], PLACE=[];
const PROJECTS_GID=1084290859, PLACEMENTS_GID=null; // השמות עדיין ממתין ל-gid
const DEMO_PROJ=[
...
async function loadProj(){
  ...
  if(hasBackend()){try{const a=await gw({action:'get',sheetId:SHEET_MANAGERS,gid:PROJECTS_GID});if(a&&a.ok){PROJ=a.rows;renderMgr();return}else if(a&&a.error){PROJ=DEMO_PROJ;renderMgr();_busyNote('⚠ פרויקטים: '+a.error);return}}catch(e){}}
...
async function loadPlace(){
  ...
  if(hasBackend()){try{const a=await gw({action:'get',sheetId:SHEET_MANAGERS,gid:PLACEMENTS_GID});if(a&&a.ok){PLACE=a.rows;renderMgr();return}else if(a&&a.error){PLACE=DEMO_PLACE;renderMgr();_busyNote('⚠ השמות: '+a.error);return}}catch(e){}}
...
async function loadMgr(){
  if(hasBackend()){try{const a=await gw({action:'get',table:'',sheetId:SHEET_MANAGERS});if(a&&a.ok){MGRS=a.rows;renderMgr();return}else if(a&&a.error){MGRS=DEMO_MGRS;renderMgr();const bz=document.getElementById('busy');if(bz){bz.style.display='inline-block';bz.textContent='⚠ מנהלי עבודה: '+a.error}return}}catch(e){}}
